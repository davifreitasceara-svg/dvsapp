...
// Target the specific section using a very unique anchor
...
// Find where the old sharing block starts and where it ends
// It ends just before the </div> that closes the Criador component's main block
const searchStart = content.indexOf(startMarker);
if (searchStart !== -1) {
    const endMarker = '<div style={{ textAlign: "center", fontSize: 10, color: D.w3, marginTop: 4 }}>';
    const searchEnd = content.indexOf(endMarker);
    if (searchEnd !== -1) {
        content = content.substring(0, searchStart + startMarker.length) + replacement + content.substring(searchEnd);
    }
}

fs.writeFileSync(path, content, 'utf8');
console.log("Surgical Share UI update applied!");
