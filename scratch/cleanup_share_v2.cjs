...
const search = 'TURBINAR PARA VIRALIZAR';
const index = content.indexOf(search);

if (index !== -1) {
    const buttonEnd = content.indexOf('</button>', index) + 9;
    const nextDivEnd = content.indexOf('DVS simulará o app original');
    const cutStart = buttonEnd;
    const cutEnd = content.lastIndexOf('<div', nextDivEnd);

    content = content.substring(0, cutStart) + newShareBox + '\n        ' + content.substring(cutEnd);
